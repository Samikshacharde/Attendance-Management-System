...
def ADD_STUDENT(request):
    ...
    if request.method == "POST":
        ...
        if CustomUser.objects.filter(username=username).exists():
            ...
            student.save()
            messages.success(request, user.first_name + " " + last_name + " is successfully added")
            return redirect('add_student')
    ...
    if request.method == 'POST':
        ...
        if CustomUser.objects.filter(username=username).exists():
            ...
            user = CustomUser(first_name=first_name, last_name=last_name, email=email
                              , profile_pic=profile_pic, username=username, user_type=2)
            user.set_password(password)
            user.save()
            ...
            staff.save()
            messages.success(request, 'Staff Succesfully added !')
...
def DELETE_SUBJECT(request, id):
    ...
    subject.delete()
    messages.success(request, 'Subject Record successfully Deleted!')
    return redirect('view_subject')
